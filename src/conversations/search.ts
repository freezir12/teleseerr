...
export class SearchConversation {
  ...
  private async handleResults(): Promise<void> {
    ...
    if (this.ctx.callbackQuery?.data === "request") {
      ...
      console.log(`🎯 [SearchConversation] Processing request for ${mediaType} ID: ${mediaId}`);
      
      try {
        // For TV shows, we'll request all seasons by default
        // You can modify this to ask the user which seasons they want
        const requestOptions = mediaType === "tv" ? { seasons: "all" as const } : undefined;
        
        const ok = await request(mediaType, mediaId, requestOptions);

        if (ok) {
          await this.ctx.api.deleteMessage(this.ctx.chatId!, this.messageId);
          await this.ctx.reply("✅ Requested successfully!");
          return;
        }

        await this.ctx.reply("❌ Something went wrong with the request");
        return;
      } catch (error) {
        console.error(`💥 [SearchConversation] Request failed:`, error);
        const errorMessage = error instanceof Error ? error.message : String(error);
        await this.ctx.reply(`❌ Request failed: ${errorMessage}`);
        return;
      }
    }
